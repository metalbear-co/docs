...
Configuration options are listed [here](/reference/configuration). The configuration file also supports autocomplete when edited in VS Code when the extension is installed.
...
Configuration options are listed [here](/reference/configuration).
...
## Test it out!
Now that you've installed the CLI tool or one of the extensions, lets see mirrord at work. By default, mirrord will mirror incoming traffic to the remote target (this can be changed in the [configuration](/reference/configuration/#feature-network-incoming-mode)), sending a duplicate to the same port on your local process. So if your remote target receives traffic on port 80, your local process will receive a copy of that traffic on that same port (this can also be [configured](/reference/configuration/#feature-network-incoming-port_mapping)). 
...
We find that this configuration works for a lot of use cases, but if you'd like to change it, please read about available options in the [configuration](/reference/configuration/).
...
3. If you don't want to impersonate a remote target - for example, if you want to run a tool in the context of your cluster - check out our [guide on the targetless mode](/using-mirrord/targetless/).
4. If you just want to learn more about mirrord, why not check out our [architecture](/reference/architecture) or [configuration](/reference/configuration) sections?
