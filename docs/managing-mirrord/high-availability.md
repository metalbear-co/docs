...
By default, the mirrord Operator workload runs with a single replica.
Starting from chart version `1.40.3`, this can be configured with the `operator.replicas` setting in the chart.

Regardless of the configured scale, only one replica acts as a leader, and serves mirrord sessions.
Any additional replicas wait in a standby mode, ready to acquire leadership and resume work, in case of the current leader's failure.
