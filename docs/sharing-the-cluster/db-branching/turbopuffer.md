...
{% hint style="info" %}
turbopuffer branching requires operator `3.210.0`, mirrord CLI `3.262.0`, and operator Helm chart `3.210.0` with the `operator.turbopufferBranching` value set to `true`.
{% endhint %}
...
| `base_url` | one of the two | no | The full API endpoint instead of a region, for dedicated clusters. |
