...
Stored functions and procedures, dropping `DEFINER`, and mirroring the source's server settings require mirrord operator `3.210.0` or later. Earlier operators copy tables, data, views and triggers, but leave the source's `DEFINER` in place, so any object carrying one fails to import.
