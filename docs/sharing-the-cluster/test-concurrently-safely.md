...
Allows the operator to control which patterns may be used as [HTTP header filters](../networking/filter-incoming-traffic.md#filtering-a-subset-of-traffic-with-mirror-or-steal-mode). Header filters specified by the user must match the regex specified in the network policy.
...
Allows the operator to enforce using a [mirrord profile](standardise-dev-config.md) and to specify a set of allowed profiles.
