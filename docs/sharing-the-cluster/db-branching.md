...
| `migrations` | (MySQL, PostgreSQL, MSSQL & ClickHouse only) Automatically run schema migrations on the branch so it comes up with the schema your code expects. See [Schema Migrations](db-branching/migrations.md) for details. |
