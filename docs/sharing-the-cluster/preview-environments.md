...
![How mirrord Preview Environments work works](/docs/sharing-the-cluster/preview-envs/images/preview-envs-diagram.png)
...
At this point, the preview pod is running in the cluster and receiving traffic according to your configuration. You can share the key with teammates so they can route their requests to the preview, for example by using the [browser extension](../using-mirrord/incoming-traffic/debug-from-browser.md).
...
```bash
$ mirrord preview stop --key key-my-feature
...
- Targetless mode not yet supported: preview environments currently require a target workload to clone the pod spec from. Support for [targetless](../using-mirrord/targetless.md) preview environments, creating a fresh, isolated pod without an existing target, is not yet available.
