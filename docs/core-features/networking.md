...
**When you need more control:** To steal only specific requests (by header, path, or method) while letting other traffic flow to the target normally, see [Traffic Filtering](../using-mirrord/traffic-filtering.md).
...
**When you need more control:** If you want some connections to stay local (e.g., a local database) while others go through the cluster, see [Outgoing Filter](../using-mirrord/outgoing-filter.md).

## DNS resolution

Cluster service names resolve correctly because mirrord forwards DNS queries to the cluster.

When your app looks up `payment-service.payments.svc.cluster.local`, mirrord resolves it using the cluster's DNS, returning the in-cluster IP.
