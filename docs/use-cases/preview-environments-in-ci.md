...
| `preview start` succeeds but the app doesn't respond | `preview start` waits for the readiness probes copied from the target. A target container with no `readinessProbe` counts as ready as soon as it starts. Add a `readinessProbe` to the target so a broken image fails the job. See [Readiness](preview-environments.md#readiness). |
