...
Today, the extension lets you inject a header into browser requests to route traffic to your local service. The next version goes further - it connects to the Operator and shows every active session across your
...
- Per-tab indicator showing which key is currently active on that tab
- Session-sharing via URL, get a shareable link that auto-configures the extension for a colleague
...
## `mirrord up`: Multi-Service Session Launcher
...
- `mirrord down` tears down all sessions associated with the config
