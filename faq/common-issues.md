...
Another reason that mirrord might seem not to work is if your remote pod has more than one container. mirrord works at the level of the container, not the whole pod. If your pod runs multiple containers, you need to make sure mirrord targets the correct one by by specifying it explicitly in the [target configuration](/reference/configuration/#root-target). Note that we filter out the proxy containers added by popular service meshes automatically.
...
This could happen because the local process is listening on a different port than the remote target. You can either change
 ...
 ](/reference/configuration/#feature-network-incoming-port_mapping) to map the local port to a
 remote port.
...
immediately (or after a [configurable TTL](/reference/configuration/#agent-ttl)).
...
When running the agent as an [ephemeral container](/reference/configuration#agent-ephemeral), the agent shares the network stack with the target pod.
