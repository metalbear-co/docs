...
Allows the operator to control which patterns may be used as [HTTP header filters](/using-mirrord/steal/#stealing-only-a-subset-of-the-remote-targets-traffic).
...
Allows the operator to enforce using a [mirrord profile](/managing-mirrord/profiles) and to specify a set of allowed profiles.
