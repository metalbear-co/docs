...
When a target is specified, a [mirrord-agent](/reference/architecture/#mirrord-agent) pod will be created on the same
...
### Using a [configuration file](/reference/configuration)

The target path from the last section is set under the
[`target.path`](/reference/configuration/#target-path) field. The target's namespace can be set
under [`target.namespace`](/reference/configuration/#target-namespace). By default, the namespace
...
> target namespace. That value can be set via the [`agent.namespace` configuration file field](/reference/configuration#agent-namespace), the `-a` CLI argument,
