...
![Select Active Config action](/using-mirrord/intellij-plugin/images/enabler.png)
...
1. with the target selection dialog
    ...
    - The dialog will initially show targets in the namespace specified in the mirrord config ([`.target.namespace`](/reference/configuration/#target-namespace)). 
...
The plugin allows for using the [mirrord config](/reference/configuration).
...
![Select Active Config action](/using-mirrord/intellij-plugin/images/select-active-config.png)
...
The guide on how to use the plugin with remote development on WSL can be found [here](/using-mirrord/wsl/#root-project-intellij).
