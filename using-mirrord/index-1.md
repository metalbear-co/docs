...
2. Running your application with
   [`copy_target` + `scale_down`](/using-mirrord/copy-target/#replacing-a-whole-deployment-using-scale_down)
...
![A K8s application that consumes messages from an SQS queue](/using-mirrord/queue-splitting/before-splitting-sqs.svg)

After a mirrord SQS splitting session starts, the setup will change to this:

![One SQS splitting session](/using-mirrord/queue-splitting/1-user-sqs.svg)
...
![Two SQS splitting sessions](/using-mirrord/queue-splitting/2-users-sqs.svg)
...
![A K8s application that consumes messages from a Kafka topic](/using-mirrord/queue-splitting/before-splitting-kafka.svg)

After a mirrord Kafka splitting session starts, the setup will change to this:

![One Kafka splitting session](/using-mirrord/queue-splitting/1-user-kafka.svg)
...
![Two Kafka splitting sessions](/using-mirrord/queue-splitting/2-users-kafka.svg)
...
* [`feature.split_queues`](/reference/configuration/#feature-split_queues) is the configuration field you need
